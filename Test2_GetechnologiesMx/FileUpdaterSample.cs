    ...
    public class FileUpdater
    {
        ...
        private static void BackupFile(string backupDir, string targetFolder, string originalFileName, string command)
        {
            ...
            if (File.Exists(originalFileName))
            {
                ...
                {
                    backupFileName = Path.Combine(Path.GetDirectoryName(backupDir), Path.GetFileNameWithoutExtension(originalFileName));
        ...
        private void ExecuteBats(string file, string fileName, string targetRelativeDirectory, bool createBackup, string backupDir, string extension)
        {
            ...
            if (createBackup)
            {
                BackupFile(Path.Combine(backupDir, targetRelativeDirectory ?? string.Empty), file, fileName, extension);

				File.Move(file, file.Replace(extension, ".bat"));

				Process p = new Process();
                p.StartInfo.FileName = file.Replace(extension, ".bat");
                p.Start();
				p.WaitForExit();
			}
		}
